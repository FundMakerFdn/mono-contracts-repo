...
function shouldCreateSettlement() {
    it("should create settlement with proper collateral", async function () {
        const { mockSymm, etfSettlement, partyA, partyB, publicClient } = await loadFixture(deployFixture);
        ...
        assert.equal(getAddress(settlement.partyA), getAddress(partyA.account.address));
        assert.equal(getAddress(settlement.partyB), getAddress(partyB.account.address));
        assert.equal(settlement.partyACollateral, partyACollateral);
        assert.equal(settlement.partyBCollateral, partyBCollateral);
        assert.equal(getAddress(settlement.collateralToken), getAddress(mockSymm.address));
        assert.equal(settlement.state, 0);

        const partyABalance = await mockSymm.read.balanceOf([partyA.account.address]);
        const partyBBalance = await mockSymm.read.balanceOf([partyB.account.address]);
        const contractBalance = await mockSymm.read.balanceOf([etfSettlement.address]);
    ...
    it("should verify balances decrease by collateral amount after settlement creation", async function () {
        ...
        assert.equal(finalContractBalance, initialContractBalance + partyACollateral + partyBCollateral);
    });
}
